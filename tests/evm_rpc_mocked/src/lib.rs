...
use ic_cdk_macros::*;
use evm_rpc_types::{BlockTag, GetLogsArgs, Hex, Hex20, Hex256, Hex32, LogEntry, MultiRpcResult, Nat256, RpcConfig, RpcServices
};
...
async fn get_same_logs_as_sub_filters() -> Vec<LogEntry> {
    ...
    let result: Result<(Vec<SubscriptionInfo>,), _> =
        call(evm_logs_canister, "get_subscriptions", (None::<u32>, None::<Nat>, None::<Vec<Filter>>)).await;
