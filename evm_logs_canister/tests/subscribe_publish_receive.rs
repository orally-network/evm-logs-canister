...
async fn test_event_publishing_and_notification_delivery() {
  ...
  pic
    .install_canister(subscriber_canister_id, subscriber_wasm_bytes.to_vec(), vec![], None)
    .await;
  ...
  pic
    .install_canister(proxy_canister_id, proxy_wasm_bytes.to_vec(), vec![], None)
    .await;
  ...
  pic
    .install_canister(evm_logs_canister_id, evm_logs_wasm_bytes.to_vec(), init_args, None)
    .await;
  ...
  let cycles_wallet_wasm_path = std::env::var("CYCLES_WALLET_WASM_PATH").expect("CYCLES_WALLET_WASM_PATH must be set");
  let cycles_wallet_wasm_bytes = tokio::fs::read(cycles_wallet_wasm_path)
    .await
    .expect("Failed to read the cycles wallet WASM file");

  pic
    .install_canister(cycles_wallet_id, cycles_wallet_wasm_bytes.to_vec(), vec![], None)
    .await;
